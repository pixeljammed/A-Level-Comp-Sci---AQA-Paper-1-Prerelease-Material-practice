...
namespace TargetClearCS
{
    public class Q9
    {
        ...
        {
            int Score = 0;
            int PositionToFreeze;
            int FrozenNumberIndex = -1;
            bool GameOver = false;
            string UserInput;
            List<string> UserInputInRPN;
            while (!GameOver)
            {
                DisplayState(Targets, NumbersAllowed, Score, FrozenNumberIndex);
                Console.Write("Would you like to freeze a number? (y/n): ");
                UserInput = Console.ReadLine();
                if (UserInput.ToLower() == "y")
                {
                    Console.Write("Enter number position you want to freeze. Enter nothing to unfreeze:");
                    PositionToFreeze = Convert.ToInt32(Console.ReadLine());
                    if (PositionToFreeze is int && PositionToFreeze > 0 && PositionToFreeze < Targets.Count)
                    {
                        FrozenNumberIndex = PositionToFreeze;
                    }
                    else FrozenNumberIndex = -1;
                    DisplayState(Targets, NumbersAllowed, Score, FrozenNumberIndex);
                }
                ...
                {
                    UpdateTargets(Targets, TrainingGame, MaxTarget, FrozenNumberIndex);
                }
            }
        ...
        static void UpdateTargets(List<int> Targets, bool TrainingGame, int MaxTarget, int FrozenNumberIndex)
        {
            int Swap;
            ...
            if (FrozenNumberIndex != -1)
            {
                Swap = Targets[FrozenNumberIndex - 1];
                Targets[FrozenNumberIndex - 1] = Targets[FrozenNumberIndex];
                Targets[FrozenNumberIndex] = Swap;
            }
        }
        ...
        static void DisplayState(List<int> Targets, List<int> NumbersAllowed, int Score, int FrozenNumberIndex)
        {
            DisplayTargets(Targets, FrozenNumberIndex);
        ...
        static void DisplayTargets(List<int> Targets, int FrozenNumberIndex)
        {
            Console.Write("|");
            for (int T = 0; T < Targets.Count; T++)
            {
                ...
                else if (T == FrozenNumberIndex)
                {
                    Console.Write($"<{Targets[T]}>");
                }
                else
                {
                    Console.Write(Targets[T]);
