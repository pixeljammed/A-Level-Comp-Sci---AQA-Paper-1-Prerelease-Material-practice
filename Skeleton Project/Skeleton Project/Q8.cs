    ...
    public class Q8
    {
        ...
        static void PlayGame(List<int> Targets, List<int> NumbersAllowed, bool TrainingGame, int MaxTarget, int MaxNumber)
        {
            ...
            while (!GameOver)
            {
                ...
                {
                    ChallengeMode = true;
                    Console.WriteLine("CHALLENGE MODE! Use all of the numbers possible for a 10 point bonus - if you don't, you'll lose 5 points!");
                }
                else
                {
                    ChallengeMode = false;
                ...
                if (CheckIfUserInputValid(UserInput))
                {
                    ...
                    if (CheckNumbersUsedAreAllInNumbersAllowed(NumbersAllowed, UserInputInRPN, MaxNumber))
                    {
                        if (CheckIfUserInputEvaluationIsATarget(Targets, UserInputInRPN, ref Score))
                        {
                            ...
                            ChallengeModeCheck(NumbersAllowed, ref Score);
                            NumbersAllowed = FillNumbers(NumbersAllowed, TrainingGame, MaxNumber);
                        }
        ...
        static void ChallengeModeCheck(List<int> NumbersAllowed, ref int Score)
        {
            if (NumbersAllowed.Count == 0)
            {
                Console.WriteLine("You used all the numbers! +10 points");
                Score += 10;
            }
            else
            {
                Console.WriteLine("You didn't use all the numbers! -5 points!");
                Score -= 5;
            }
        }
